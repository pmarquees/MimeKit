...
- Next.js App Router Application (Frontend web application server with SSR/SSG capabilities) -> Next.js 15, React 19, TypeScript
- tRPC API Layer (Type-safe API layer for client-server communication) -> tRPC v11, React Query, Zod
- Authentication System (User authentication and session management) -> NextAuth.js v5, bcryptjs, JWT
- PostgreSQL Database (Primary data store for all application data) -> PostgreSQL, Prisma ORM
- Prisma ORM (Database access layer and schema management) -> Prisma Client, Prisma Migrate
- Cloudflare R2 Storage (Object storage for user uploads (images, videos, files)) -> Cloudflare R2, AWS SDK S3
- Rich Text Editor (Client-side WYSIWYG editor for posts and comments) -> Lexical, Custom plugins, Markdown support
- Webhook Integration Service (Send notifications to external platforms) -> Discord webhooks, Slack webhooks
...
- nextjs-app -> auth-system (request)
- auth-system -> prisma-orm (request)
- lexical-editor -> trpc-api (request)
- trpc-api -> r2-storage (request)
- nextjs-app -> r2-storage (request)
- trpc-api -> webhook-integration (event)
- notification-system -> prisma-orm (request)
...
- Prisma ORM manages persistent state via Prisma Client, Prisma Migrate.
...
- Map contract to stored model: Prisma ORM manages persistent state via Prisma Client, Prisma Migrate.
