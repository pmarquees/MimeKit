...
- Next.js App Router (Web application framework serving SSR and client-side React pages) -> Next.js 15, React 19, TypeScript
- tRPC API Layer (Type-safe API layer providing backend procedures for data operations) -> tRPC v11, React Query, Zod
- NextAuth Authentication (Handles user authentication with credentials provider and JWT sessions) -> NextAuth v5, bcryptjs, JWT
- Prisma Database Layer (ORM managing database schema and queries for PostgreSQL) -> Prisma 6.8.2, PostgreSQL
- PostgreSQL Database (Primary data store for users, posts, projects, comments, reactions, and notifications) -> PostgreSQL
- Cloudflare R2 Storage (S3-compatible object storage for images, videos, and file attachments) -> Cloudflare R2, AWS SDK S3 Client
- Lexical Rich Text Editor (Rich text editing component with mentions, attachments, markdown shortcuts) -> Lexical, Custom plugins, React
- shadcn/ui Component Library (Reusable UI components for consistent design system) -> Radix UI, Tailwind CSS v4, shadcn/ui
...
#### Interfaces
- nextjs-app -> trpc-api (request)
- nextjs-app -> auth-layer (request)
- trpc-api -> prisma-orm (request)
- prisma-orm -> postgres-db (data)
- trpc-api -> upload-handler (request)
- upload-handler -> r2-storage (request)
- lexical-editor -> nextjs-app (event)
- ui-components -> nextjs-app (event)
- trpc-api -> webhook-service (event)
- trpc-api -> search-service (request)
...
- Prisma Database Layer manages persistent state via Prisma 6.8.2, PostgreSQL.
- Cloudflare R2 Storage manages persistent state via Cloudflare R2, AWS SDK S3 Client.
...
- Map contract to stored model: Prisma Database Layer manages persistent state via Prisma 6.8.2, PostgreSQL.
- Map contract to stored model: Cloudflare R2 Storage manages persistent state via Cloudflare R2, AWS SDK S3 Client.
